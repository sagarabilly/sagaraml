...
def loss_fn(output, groundtruth):
        ...
        if val_loader is not None:
            ...
            with torch.no_grad():
                for X_val, y_val in val_loader:
                    ...
                    running_val_loss += loss_fn(y_val_pred, y_val).item() * X_val.size(
                        0
                    )
...
def prepare_sequences(X, y, seq_len):
    ...
    # Reshaping the data
    X_sequences = X[: num_sequences * seq_len].reshape(
        num_sequences, seq_len, X.shape[1]
    )
    y_sequences = y[: num_sequences * seq_len].reshape(num_sequences, seq_len, 1)[
        :, -1, :
    ]
